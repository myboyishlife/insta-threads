...
Include 3-5 relevant hashtags."""
    ...
    def get_page_access_token(self):
        ...
        try:
            ...
            self.send_message(f"Page ID {self.fb_page_id} not found in user's account list.", level=logging.ERROR, immediate=True)
            return None
    ...
    def post_to_instagram(self, dbx, file, caption, page_token, total_files=None):
        ...
        self.log_console_only(f"Instagram: {media_type} Size:{filesize} Remaining:{total_files}", level=logging.INFO)
        ...
        request_time = time.time() - start_time
        self.log_console_only(f"API request completed in {request_time:.2f} seconds", level=logging.INFO)
        self.log_console_only(f"Response status: {res.status_code}", level=logging.INFO)
        ...
        if media_type == "REELS":
            ...
            for attempt in range(self.INSTAGRAM_REEL_STATUS_RETRIES):
                ...
                status_response = self.session.get(
                    f"{self.INSTAGRAM_API_BASE}/{creation_id}?fields=status_code",
                ...
                self.log_console_only(f"Waiting {self.INSTAGRAM_REEL_STATUS_WAITTIME} seconds...", level=logging.INFO)
                time.sleep(self.INSTAGRAM_REEL_STATUS_WAITTIME)
        ...
        for attempt in range(self.INSTAGRAM_PUBLISH_ATTEMPTS):
            if time.time() - start_time > self.PUBLISH_MAX_WAITTIME:
                self.send_message(f"Instagram publish timeout after {self.PUBLISH_MAX_WAITTIME}s", level=logging.ERROR, immediate=True)
                return False
            ...
            if pub.status_code == 200:
                ...
                if not instagram_id:
                    ...
                    self.send_message(f"Instagram published! Media ID: {instagram_id} Remaining: {total_files - 1}", immediate=True)
    ...
    def post_to_facebook_page(self, dbx, file, caption, page_token):
        ...
        if not self.fb_page_id:
            self.send_message("Facebook Page ID not configured, skipping Facebook post", level=logging.WARNING, immediate=True)
            return False
    ...
    def post_facebook_reel(self, file, media_url, caption, page_token):
        ...
        for attempt in range(self.FACEBOOK_PUBLISH_ATTEMPTS):
            if time.time() - start_time > self.PUBLISH_MAX_WAITTIME:
                self.send_message(f"Facebook Reel publish timeout after {self.PUBLISH_MAX_WAITTIME}s", level=logging.ERROR, immediate=True)
                return False
            ...
            if finish_res.status_code == 200:
                ...
                self.send_message(f"Facebook Reel published! Video ID: {fb_video_id}", immediate=True)
    ...
    def post_facebook_video(self, file, media_url, caption, page_token):
        ...
        for attempt in range(self.FACEBOOK_PUBLISH_ATTEMPTS):
            if time.time() - start_time > self.PUBLISH_MAX_WAITTIME:
                self.send_message(f"Facebook video publish timeout after {self.PUBLISH_MAX_WAITTIME}s", level=logging.ERROR, immediate=True)
                return False
            ...
            if res.status_code == 200:
                ...
                self.send_message(f"Facebook video published! Video ID: {video_id}", immediate=True)
    ...
    def post_to_threads(self, dbx, file, caption, total_files=None):
        ...
        self.send_message(f"Uploading to Threads: {file.name} Type:{media_type} Remaining:{total_files}", immediate=True)
        ...
        for attempt in range(self.THREADS_PUBLISH_ATTEMPTS):
            if time.time() - start_time > self.PUBLISH_MAX_WAITTIME:
                self.send_message(f"Threads publish timeout after {self.PUBLISH_MAX_WAITTIME}s", level=logging.ERROR, immediate=True)
                return False
        ...
        try:
            ...
            for attempt in range(self.VERIFY_ATTEMPTS):
                ...
                if attempt < self.VERIFY_ATTEMPTS - 1:
                    ...
                    self.log_console_only(f"Retrying in {backoff_time}s...", level=logging.INFO)
                    time.sleep(backoff_time)
            
            self.send_message(f"Could not verify {platform_name} post is live after {self.VERIFY_ATTEMPTS} attempts", level=logging.WARNING)
            return False
